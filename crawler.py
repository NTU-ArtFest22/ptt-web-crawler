    ...
    if args.i:
        ...
        for i in range(end - start + 1):
            ...
            for div in divs:
                try:
                    # ex. link would be <a
                    # href="/bbs/PublicServan/M.1127742013.A.240.html">Re: [問題]
                    # 職等</a>
                    href = div.find('a')['href']
                    link = PTT_URL + href
                    article_id = re.sub('\.html', '', href.split('/')[-1])
                    if div == divs[-1] and i == end - start:  # last div of last page
                        store(filename, parse(
                            link, article_id, board) + '\n', 'a')
                    else:
                        store(filename, parse(
                            link, article_id, board) + ',\n', 'a')
                except:
...
def parse(link, article_id, board):
    ...
    date = ''
    if metas:
        author = metas[0].select('span.article-meta-value')[0].string if metas[
            0].select('span.article-meta-value')[0] else author
        title = metas[1].select('span.article-meta-value')[0].string if metas[
            1].select('span.article-meta-value')[0] else title
        date = metas[2].select('span.article-meta-value')[0].string if metas[
            2].select('span.article-meta-value')[0] else date
    ...
    filtered = [v for v in main_content.stripped_strings if v[
        0] not in [u'※', u'◆'] and v[:2] not in [u'--']]
    expr = re.compile(
        u(r'[^\u4e00-\u9fa5\u3002\uff1b\uff0c\uff1a\u201c\u201d\uff08\uff09\u3001\uff1f\u300a\u300b\s\w:/-_.?~%()]'))
    ...
    for push in pushes:
        ...
        # if find is None: find().strings -> list -> ' '.join; else the current
        # way
...
def getLastPage(board):
    content = urlopen('https://www.ptt.cc/bbs/' + board +
                      '/index.html').read().decode('utf-8')
    first_page = re.search(r'href="/bbs/' + board +
                           '/index(\d+).html">&lsaquo;', content).group(1)
